	...
	case GS_MJ_FREE:	//空闲状态
		{
			...
			int nRandTime = 6 + (rand() % __max(1, m_cbTimeStartGame));
#ifdef _DEBUG
			//nRandTime = 1;
#endif // _DEBUG
	...
	case GS_MJ_CALL:	//游戏状态
		{
			...
			//扑克变量
			m_GameLogic.SwitchToCardIndex(pStatusCall->cbCardData, pStatusCall->cbCardCount[m_pIAndroidUserItem->GetChairID()], m_cbCardIndex);
			...
			if (m_wBankerUser == m_pIAndroidUserItem->GetChairID())
			{
				...
				{
					//变量定义
					BYTE cbCardCount = pStatusCall->cbCardCount[m_pIAndroidUserItem->GetChairID()];
					BYTE cbRemoveCard[] = {pStatusCall->cbSendCardData};

					//调整扑克
					m_GameLogic.RemoveCard(pStatusCall->cbCardData, cbCardCount, cbRemoveCard, 1);
					pStatusCall->cbCardData[pStatusCall->cbCardCount[m_pIAndroidUserItem->GetChairID()] - 1] = pStatusCall->cbSendCardData;
				}
			}
	...
	case GS_MJ_PLAY:	//游戏状态
		{
			...
			m_GameLogic.SwitchToCardIndex(pStatusPlay->cbCardData, pStatusPlay->cbCardCount[m_pIAndroidUserItem->GetChairID()], m_cbCardIndex);
			...
			if (m_wCurrentUser==m_pIAndroidUserItem->GetChairID())
			{
				...
				{
					//变量定义
					BYTE cbCardCount = pStatusPlay->cbCardCount[m_pIAndroidUserItem->GetChairID()];
					BYTE cbRemoveCard[]={pStatusPlay->cbSendCardData};

					//调整扑克
					m_GameLogic.RemoveCard(pStatusPlay->cbCardData,cbCardCount,cbRemoveCard,1);
					pStatusPlay->cbCardData[pStatusPlay->cbCardCount[m_pIAndroidUserItem->GetChairID()] - 1] = pStatusPlay->cbSendCardData;
				}
			}
