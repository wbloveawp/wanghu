...
bool CTableFrameSink::OnEventGameStart()
{
	...
	//m_cbCardIndex[m_wBankerUser][m_GameLogic.SwitchToCardIndex(m_cbSendCardData)]--;//只发10张	
	//发送数据
	for (WORD i = 0; i < GAME_PLAYER; i++)
	{
	...
	//BYTE cbIDITime = 10;//m_cbUserAction[m_wBankerUser] == WIK_NULL ? m_pGameCustomRule->cbTimeOutCard + 1 : m_pGameCustomRule->cbTimeOperateCard + 1;
	...
	//听牌判断
	/*if (m_GameLogic.AnalyseTingCard(m_cbCardIndex[m_wBankerUser], 0, 0, 0))
	{
...
bool CTableFrameSink::OnEventGameConclude(WORD wChairID, IServerUserItem* pIServerUserItem, BYTE cbReason)
{
	...
	case GER_NORMAL:		//常规结束
	{
		...
		m_pITableFrame->SetGameTimer(IDI_GAMEEND, 5 * 1000, 1, 0);
...
//发送场景
bool __cdecl CTableFrameSink::OnEventSendGameScene(WORD wChiarID, IServerUserItem* pIServerUserItem, BYTE cbGameStatus, bool bSendSecret)
{
	...
	/*	CString cs;
		cs.Format(TEXT("wHeapHead = %d, wHeapTail = %d, lCellScore：%Id,wCurrentUser:%d ,cbOutCardData:%d,cbSendCardData:%d,cbLeftCardCount%d,【 %d，%d，%d，%d，  %d，%d，%d，%d】"), StatusPlay.wHeapHand, StatusPlay.wHeapTail, StatusPlay.lCellScore, StatusPlay.wCurrentUser, StatusPlay.cbOutCardData, StatusPlay.cbSendCardData, StatusPlay.cbLeftCardCount,StatusPlay.cbHeapCardInfo[1][1], StatusPlay.cbHeapCardInfo[1][2], StatusPlay.cbHeapCardInfo[2][1], StatusPlay.cbHeapCardInfo[2][2], StatusPlay.cbHeapCardInfo[3][1], StatusPlay.cbHeapCardInfo[3][2], StatusPlay.cbHeapCardInfo[4][1], StatusPlay.cbHeapCardInfo[4][2]) ;
		CTraceService::TraceString(cs,TraceLevel_Exception);*/
...
bool CTableFrameSink::OnUserOperateCard(WORD wChairID, BYTE cbOperateCode, BYTE cbOperateCard)
{
	...
	if (m_wCurrentUser == INVALID_CHAIR)
	{
		...
		case WIK_PENG:		//碰牌操作
		{
			...
			BYTE cbIDITime = 30;//m_pGameCustomRule->cbTimeOutCard + 1;
			m_pITableFrame->SetGameTimer(IDI_SO_OPERATE, cbIDITime * 1000, 1, 0);
...
bool CTableFrameSink::DispatchCardData(WORD wCurrentUser, bool bTail)
{
	...
	BYTE cbIDITime = 30;//m_cbUserAction[m_wCurrentUser] == WIK_NULL ? m_pGameCustomRule->cbTimeOutCard + 1 : m_pGameCustomRule->cbTimeOperateCard + 1;
	m_pITableFrame->SetGameTimer(IDI_SO_OPERATE, cbIDITime * 1000, 1, 0);
...
bool CTableFrameSink::EstimateUserRespond(WORD wCenterUser, BYTE cbCenterCard, enEstimatKind EstimatKind)
{
	...
	if (bAroseAction == true)
	{
		...
		BYTE cbIDITime = 30;// m_pGameCustomRule->cbTimeOperateCard + 1;
		m_pITableFrame->SetGameTimer(IDI_SO_OPERATE, cbIDITime * 1000, 1, 0);
