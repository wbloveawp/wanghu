...
#define GS_MJ_GAMEEND				GAME_STATUS_PLAY+3					//结算状态

//常量定义
#define MAX_WEAVE				4									//最大组合
#define MAX_INDEX					27+1								//最大索引（没有字牌和风牌）
...
#define SUB_S_TING_DATA				118									//听牌数据
#define SUB_S_HU_DATA				119									//胡牌数据
#define SUB_S_USER_GIVEUP			120									//玩家携带输完
#define SUB_S_FREE_START			121									//空闲开始

struct CMD_S_UserGiveup
{
	WORD						wChairID;
};
//空闲状态
...
struct CMD_S_StatusPlay
{
	...
	LONGLONG							lHuScore[GAME_PLAYER];				//游戏胡牌分数
	...
	WORD							wServerLevel;						//房间等级
	BYTE							cbTimeRemain;						//重连剩余秒数
	bool							cbPlayerStatus[GAME_PLAYER];		//玩家游戏状态

	bool                            bFirstHu[GAME_PLAYER];
};
...
struct CMD_S_GameEnd
{
	...
	tagChart						tagHistoryChart[GAME_PLAYER][MAX_CHART_COUNT];	//对局流水清单
	bool							cbPlayerGiveup[GAME_PLAYER];		//认输玩家
};

//结算状态
struct CMD_S_StatusGameEnd : public CMD_S_StatusPlay
{
	//
...
struct CMD_S_TING_DATA
{
	...
	BYTE							cbHuCardCount[MAX_COUNT];							//听后能胡哪几张牌
	BYTE							cbHuCardData[MAX_COUNT][HEAP_FULL_COUNT];
	BYTE							cbHuCardRemainingCount[MAX_COUNT][HEAP_FULL_COUNT]; //胡牌剩余数
	BYTE							cbHuFan[MAX_COUNT][HEAP_FULL_COUNT];                //胡牌翻数
};

//胡牌数据
struct CMD_S_HU_DATA
{
	BYTE							cbHuCardCount;										//能胡哪几张牌
	BYTE							cbHuCardData[HEAP_FULL_COUNT];						//胡牌数据
	BYTE							cbHuCardRemainingCount[HEAP_FULL_COUNT];			//胡牌剩余数
	BYTE							cbHuFan[HEAP_FULL_COUNT];							//胡牌翻数
};
...
#define SUB_C_REQUEST_HU_DATA		17									//请求胡牌数据
