...
bool CServiceUnits::StartService()
{
	...
	//启动监控
	if (m_MonitorCenter.StartService()==false)
	{
		ConcludeService();
		return false;
	}
...
bool CServiceUnits::ConcludeService(bool bWillStop)
{
	...
	{
		//状态判断
		if (m_ServiceStatus != SERVICE_STATUS_SERVICE && m_ServiceStatus != SERVICE_STATUS_WILLSTOP)
		{
			return false;
		}
	...
	else
	{
		...
		//停止监控
		m_MonitorCenter.ConcludeService();		
...
void _cdecl CServiceUnits::SetServiceOption(tagServiceOption & ServiceOption)
{
	...
	//设置参数
	m_MonitorClient.SetServiceParameter(SERVICE_MODULE_MONITOR, m_ServiceOption.wServiceID, m_ServiceOption.szServiceName);
}
...
bool CServiceUnits::InitializeService()
{
	...
	//配置数据
	m_MonitorCenter.SetServiceOption(&m_ServiceOption);
	m_MonitorCenter.SetServerName(m_InitParameter.m_szServerName);
	m_MonitorCenter.SetAttemperEngine(m_AttemperEngine.GetInterface());
...
bool CServiceUnits::SetServiceStatus(WORD ServiceStatus)
{
	...
	{
		//错误通知
		if ((m_ServiceStatus!= SERVICE_STATUS_WILLSTOP)&&(ServiceStatus==SERVICE_STATUS_STOP))
		{
...
LRESULT CServiceUnits::OnUIControlRequest(WPARAM wParam, LPARAM lParam)
{
	...
	{
	case UI_LOAD_CONFIG_RESULT:		//加载成功
		{
			//效验消息
	...
	case UI_SERVICE_WILLSTOP_RESULT:		//预停止
		{
			//效验消息
			ASSERT(DataHead.wDataSize == sizeof(CP_ControlResult));
			...
			{
				//提示信息
				CTraceService::TraceString(TEXT("服务停止失败！"), TraceLevel_Exception);
