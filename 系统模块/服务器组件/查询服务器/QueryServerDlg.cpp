...
VOID CQueryServerDlg::OnCancel()
{
	...
	{
		LPCTSTR pszQuestion=TEXT("查询服务器正在运行中，您确实要关闭服务器吗？");
...
VOID CQueryServerDlg::SetWindowStatusText(LPCTSTR pszServiceStatus)
{
	...
	_sntprintf_s(szWindowText,CountArray(szWindowText),TEXT("查询服务器[%d] -- [ %s ]"), pServiceOption->wServicePort,pszServiceStatus);
