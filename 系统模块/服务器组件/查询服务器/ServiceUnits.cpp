...
bool CServiceUnits::ConcludeService()
{
	...
	if (m_DataCacheEngine.GetInterface() != NULL) m_DataCacheEngine->ConcludeService();
...
void CServiceUnits::SetServiceOption(tagServiceOption& ServiceOption)
{
	...
	//设置参数
	m_MonitorClient.SetServiceParameter(SERVICE_MODULE_QUERY, m_ServiceOption.wServiceID, m_ServiceOption.szServiceName);
}
...
bool CServiceUnits::InitializeService()
{
	...
	if ((m_DataCacheEngine.GetInterface()==NULL) && (m_DataCacheEngine.CreateInstance()==false)) return false;
	...
	//数据接口	
	IUnknownEx * pDataCacheEngineSink[CountArray(m_DataCacheEngineSink)];
	for (int i = 0; i < CountArray(m_DataCacheEngineSink); i++)
	{
		pDataCacheEngineSink[i] = QUERY_OBJECT_INTERFACE(m_DataCacheEngineSink[i], IUnknownEx);
	}
	...
	//配置接口
	m_DataCacheEngine->SetHashEngineKey(true);

	//绑定接口
	if (m_AttemperEngine->SetAttemperEngineSink(pIAttemperEngineSink)==false) return false;
	if (m_DataCacheEngine->SetDataBaseEngineSink(pDataCacheEngineSink,CountArray(pDataCacheEngineSink))==false) return false;
	...
	//设置回调
	m_DataCacheEngine->SetDBExceptionHookFun(&CMonitorClient::DBExceptionHookFun);

	//调度回调
	m_AttemperEngineSink.m_pServiceOption=&m_ServiceOption;
	m_AttemperEngineSink.m_pInitParameter=&m_InitParameter;
	m_AttemperEngineSink.m_pITimerEngine=m_TimerEngine.GetInterface();
	m_AttemperEngineSink.m_pIDataCacheEngine=m_DataCacheEngine.GetInterface();
	...
	for (int i = 0; i < CountArray(m_DataCacheEngineSink); i++)
	{
		m_DataCacheEngineSink[i].m_pInitParameter = &m_InitParameter;
		m_DataCacheEngineSink[i].m_pIDataBaseEngineEvent = QUERY_OBJECT_PTR_INTERFACE(pIAttemperEngine, IDataBaseEngineEvent);
	}
...
{
	//启动数据库
	if (m_DataCacheEngine->StartService() == false)
	{
		ASSERT(FALSE);
		return false;
	}
