...
bool CTCPNetworkItem::OnRecvWebSocketChanel()
{
	...
	try
	{
		while (true)
		{
			if ((!m_bHandshake || m_wRecvSize < 2) && (m_bHandshake || m_wRecvSize < 4))
				return true;
			...
			if (m_bHandshake)
			{
				...
				if (wPayloadSize < 126)
				{
					wDataSize = wPayloadSize;
				}
				else if (wPayloadSize > 126 || wPayloadSize == 126)
				{
					if (m_wRecvSize >= 4)
					{
						wDataSize = ((m_cbRecvBuf[2] << 8) | (m_cbRecvBuf[3]));
						if (wDataSize > SOCKET_TCP_BUFFER) throw TEXT("数据包超长");
						wHeadSize += 2;
					}
				}

				//整包大小 是数据包大小 + 头部
				WORD wPacketSize = wDataSize + wHeadSize;
				//不满足处理包条件
				...
				if (m_wRecvSize < wHeadSize) return false;
				...
				//二进制帧
				if (cbOpcode == FRAME_OP_BINARY && wDataSize > 0)
				{
					TCP_Head * pHead = (TCP_Head *)m_cbRecvBuf[wHeadSize];
					if (wDataSize < sizeof(TCP_Head)) throw TEXT("数据包太小");
					if (wPayloadSize > SOCKET_TCP_BUFFER)throw TEXT("数据包太长");

					//解密数据
					tagDecryptResult DecryptResult;
					ZeroMemory(&DecryptResult, sizeof(DecryptResult));
					//解密
					if (!m_Cipher.CrevasseBuffer(m_cbRecvBuf + wHeadSize, wDataSize, &DecryptResult))
					{
						throw TEXT("数据包版本不匹配");
					}
					...
					if (Command.wMainCmdID == MDM_KN_COMMAND)
					{
						...
						case SUB_KN_VALIDATE_SOCKET:	//验证
						{
							TCP_Validate * pValidate = (TCP_Validate *)DecryptResult.pData;
					...
					//移除处理过了的数据
					DWORD dwPacketSize = wHeadSize + wDataSize;
					m_wRecvSize -= dwPacketSize;
					MoveMemory(m_cbRecvBuf, m_cbRecvBuf + dwPacketSize, m_wRecvSize);
				}
			}
			//还未握手  处理握手协议
			else
			{
				...
				HttpParserWrapper.ParseHttpContent((const char *)m_cbRecvBuf, m_wRecvSize);
				...
				//移除头
				m_wRecvSize -= HttpParserWrapper.GetHeaderLength();
				MoveMemory(m_cbRecvBuf, m_cbRecvBuf + HttpParserWrapper.GetHeaderLength(), m_wRecvSize);
			}
		}
	}
	catch (...)
