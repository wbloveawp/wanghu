...
bool CAttemperEngine::OnEventTCPSocketLink(WORD wServiceID, INT nErrorCode)
{
	//缓冲锁定
	CWHDataLocker ThreadLock(m_CriticalLocker);
	NTY_TCPSocketLinkEvent pConnectEvent;
	...
	//投递数据
	return m_AsynchronismEngine.PostAsynchronismData(EVENT_TCP_SOCKET_LINK,&pConnectEvent,sizeof(NTY_TCPSocketLinkEvent));
}
...
bool CAttemperEngine::OnEventTCPSocketShut(WORD wServiceID, BYTE cbShutReason)
{
	//缓冲锁定
	CWHDataLocker ThreadLock(m_CriticalLocker);
	NTY_TCPSocketShutEvent pCloseEvent;
	...
	//投递数据
	return m_AsynchronismEngine.PostAsynchronismData(EVENT_TCP_SOCKET_SHUT,&pCloseEvent,sizeof(NTY_TCPSocketShutEvent));
}
...
bool CAttemperEngine::OnEventTCPSocketRead(WORD wServiceID, TCP_Command Command, VOID * pData, WORD wDataSize)
{
	...
	ReadEvent.wServiceID=wServiceID;
...
bool CAttemperEngine::OnEventTCPNetworkShut(DWORD dwSocketID, DWORD dwClientAddr, DWORD dwActiveTime)
{
	//缓冲锁定
	CWHDataLocker ThreadLock(m_CriticalLocker);
	NTY_TCPNetworkShutEvent pCloseEvent;
	...
	//投递数据
	return m_AsynchronismEngine.PostAsynchronismData(EVENT_TCP_NETWORK_SHUT,&pCloseEvent,sizeof(NTY_TCPNetworkShutEvent));
}
	...
	case EVENT_TCP_SOCKET_READ:		//读取事件
		{
			...
			//处理消息
			m_pIAttemperEngineSink->OnEventTCPSocketRead(pReadEvent->wServiceID,pReadEvent->Command, ((BYTE*)pData)+sizeof(NTY_TCPSocketReadEvent),pReadEvent->wDataSize);
