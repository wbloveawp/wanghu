...
#include "..\..\消息定义\CMD_StoreServer.h"
...
#define BFR_WAIT_EXPIRE				2									//等待超时
#define BFR_OFFLINE_EXPIRE			3									//掉线超时
...
//起始标识
#define TIME_TABLE_KERNEL_START		TIME_TABLE_SINK_RANGE				//起始标识
#define TIME_TABLE_EXTEND_START		(TIME_TABLE_SINK_RANGE+TIME_TABLE_KERNEL_RANGE) //起始标识
...
//桌子数量
#define MAX_TABLE_COUNT				(IDI_TABLE_FRAME_RANGE/TIME_TABLE_FRAME_RANGE)

//////////////////////////////////////////////////////////////////////////
//游戏数据标识
#define DB_GAME_REQUEST_START		0									//起始标识
#define DB_GAME_RESPOND_START		1000								//起始标识
#define DB_GAME_IDENTIFY_RANGE		1000								//标识范围

//框架数据标识
#define DB_FRAME_REQUEST_START		1000								//起始标识
#define DB_FRAME_RESPOND_START		2000								//起始标识
#define DB_FRAME_IDENTIFY_RANGE		1000								//标识范围

//扩展数据标识
#define DB_EXTEND_REQUEST_START		2000								//起始标识
#define DB_EXTEND_RESPOND_START		3000								//起始标识
#define DB_EXTEND_IDENTIFY_RANGE	1000								//标识标识
...
#define TIME_REPORT_SERVER_INFO		15L									//上报时间
...
//数据掩码
#define DATA_MASK_VIDEO				0x01								//录像数据
#define DATA_MASK_NETWORK			0x02								//网络数据
...
//积分信息
struct tagScoreInfoEx: public tagScoreInfo
{
	SCORE							lBeforScore;						//用户积分
};
...
public:
	//创建数据
	virtual VOID* CreateDataBaseEngineSink(REFGUID Guid, DWORD dwQueryVer)=NULL;
	//数据库事件
...
public:
	//分数类型
	virtual BYTE GetScoreKind()=NULL;
	//用户分数
...
public:
	...
	//修改积分
	virtual VOID ModifyUserScore(SCORE lModifyScore, const tagLockScoreInfo& LockScoreInfo)=NULL;
	//设置临时密码
...
public:
	//低保信息
	virtual tagSubsidyInfo& GetSubsidyInfo()=NULL;
	//低保次数
	...
	//低保分数
	virtual SCORE GetSubsidyScore() = NULL;
...
public:
	...
	//控制更新
	virtual bool OnEventUserControlUpdate(IServerUserItem* pIServerUserItem)=NULL;
	//控制结束
...
public:	
	...
	//kk jjia
	//qu zhuo zi canshu
	virtual CMD_GF_TableParam GetTableParam(tagScoreInfo ScoreInfoArray[], WORD wScoreCount) = NULL;
...
public:
	//发送数据
	virtual bool SendLookonData(WORD wChairID, WORD wSubCmdIDWORD, WORD wMainCmdID = MDM_GF_GAME) = NULL;
	//发送数据
	virtual bool SendLookonData(WORD wChairID, WORD wSubCmdID, VOID* pData, WORD wDataSize, WORD wMainCmdID = MDM_GF_GAME) = NULL;
	//发送数据
	virtual bool SendTableData(WORD wChairID, WORD wSubCmdID, WORD wMainCmdID = MDM_GF_GAME, BYTE cbDataMask = 0xFF) = NULL;
	//发送数据
	virtual bool SendTableData(WORD wChairID, WORD wSubCmdID, VOID* pData, WORD wDataSize, WORD wMainCmdID = MDM_GF_GAME, BYTE cbDataMask = 0xFF) = NULL;
...
public:
	//请求事件
	virtual bool PostDataBaseRequest(WORD wRequestID, DWORD dwEngineKeyID, DWORD dwTokenID) = NULL;
	//请求事件
	virtual bool PostDataBaseRequest(WORD wRequestID, DWORD dwEngineKeyID, DWORD dwTokenID, VOID* pData, WORD wDataSize) = NULL;	
...
public:
	//获取人数
	virtual WORD GetMaxPlayerCount() = NULL;
	//设置人数
	virtual VOID SetMaxPlayerCount(WORD wPlayerCount) = NULL;

	//游戏倍数
public:
	//坐下倍数
	virtual VOID SetSitdownGameMultiple(WORD wGameMultiple) = NULL;
	//起立倍数
	virtual VOID SetStandupGameMultiple(WORD wGameMultiple) = NULL;
};
...
#ifdef _UNICODE
#define VER_IGameStockQuery INTERFACE_VERSION(1,1)
static const GUID IID_IGameStockQuery = { 0x1a18e74f, 0xabf3, 0x48a6, 0x8a, 0x32, 0x8e, 0x5b, 0x9f, 0x8a, 0xaf, 0x6a };
#else
#define VER_IGameStockQuery INTERFACE_VERSION(1,1)
static const GUID IID_IGameStockQuery = { 0x9ec4d0b5, 0x3a74, 0x4392, 0xb4, 0xf3, 0x21, 0x2b, 0xcf, 0x6, 0x70, 0xc4 };
#endif

//库存查询
interface IGameStockQuery : public IUnknownEx
{
	//查询接口
public:
	//查询奖池
	virtual SCORE QueryJackpotScore() = NULL;
};
...
{
	//配牌器
	virtual bool OnEventControlSendCard(IServerUserItem * pIServerUserItem, void * pData, WORD wDataSize) = NULL;
};
...
public:
	...
	//发送数据
	virtual bool SendDataToTable(WORD wCmdTable, BYTE cbClientKind,WORD wMainCmdID, WORD wSubCmdID, VOID* pData, WORD wDataSize) = NULL;
	//发送数据
...
public:
	...
	virtual bool StartService()=NULL;
	//调试服务
	virtual bool DebugService()=NULL;
	//停止服务
	virtual bool ConcludeService()=NULL;
	//设置接口
...
#ifdef _UNICODE
#define VER_IGameDataBaseSink INTERFACE_VERSION(1, 1)
static const GUID IID_IGameDataBaseSink = { 0x42ba6f2f, 0x86ac, 0x4154, 0x8a, 0x2c, 0xb8, 0x3c, 0xc0, 0x41, 0xd3, 0xdd };
#else
#define VER_IGameDataBaseSink INTERFACE_VERSION(1, 1)
static const GUID IID_IGameDataBaseSink = { 0x7d5a209, 0xbf48, 0x48f5, 0xbb, 0x31, 0x81, 0x28, 0x77, 0x6, 0xc3, 0x24 };
#endif

//游戏数据钩子
interface IGameDataBaseSink : public IUnknownEx
{
	//配置接口
public:
	//加载配置
	virtual bool LoadGameConfig(DWORD dwConfigMask, DWORD dwTokenID) = NULL;
	//初始化
	virtual bool InitGameDataBaseSink(tagGameDBSinkParameter& GameDBSinkParameter) = NULL;
