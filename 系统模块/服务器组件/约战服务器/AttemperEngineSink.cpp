...
bool CAttemperEngineSink::VerifyTableCount(DWORD dwClubID, WORD wTableCount)
{
	...
	//CString s;
	//s.Format(_T("wMaxTableCount:%d >= wTableCount:%d"), wMaxTableCount , wTableCount);
	//CTraceService::TraceString(s, TraceLevel_Exception);
...
WORD CAttemperEngineSink::ConstructGameOption(INT& nStartIndex, WORD wModuleID, BYTE cbEmployScene, BYTE cbBuffer[], WORD wBufferSize)
{
	...
	{
		if (m_BattleGameOptionActive[i].BattleGameOption.wKindID == 98)
		{
			/*CString s;
			s.Format(_T("aaa pBattleGameOption->BattleGameOption.cbEmployScene:%d,cbEmployScene:%d,wModuleID:%d,wKindID:%d,m_BattleGameOptionActive[i].BattleGameOption.szOptionJson[0]:%s"), m_BattleGameOptionActive[i].BattleGameOption.cbEmployScene, cbEmployScene, wModuleID, m_BattleGameOptionActive[i].BattleGameOption.wKindID, string(CT2CA(m_BattleGameOptionActive[i].BattleGameOption.szOptionJson)));
			CTraceService::TraceString(s, TraceLevel_Exception);*/
		}
			...
			(wModuleID == INVALID_WORD || wModuleID == m_BattleGameOptionActive[i].BattleGameOption.wKindID))
		{
			...
			{
				CString s;
				s.Format(_T("222 pBattleGameOption->BattleGameOption.wKindID:%d,cbEmployScene:%d"), m_BattleGameOptionActive[i].BattleGameOption.wKindID, m_BattleGameOptionActive[i].BattleGameOption.cbEmployScene);
				CTraceService::TraceString(s, TraceLevel_Exception);
			}
...
WORD CAttemperEngineSink::ConstructBaseOption(INT& nStartIndex, WORD wModuleID, BYTE cbEmployScene, BYTE cbBuffer[], WORD wBufferSize)
{
	...
	{
		if (m_BattleBaseOptionActive[i].BattleOption.wKindID == 98)
		{
			CString s;
			s.Format(_T("ConstructBaseOption1 pBattleGameOption->BattleGameOption.cbEmployScene:%d,cbEmployScene:%d,wModuleID:%d,wKindID:%d"), m_BattleBaseOptionActive[i].BattleOption.cbEmployScene, cbEmployScene, wModuleID, m_BattleBaseOptionActive[i].BattleOption.wKindID);
			CTraceService::TraceString(s, TraceLevel_Exception);
		}
		// 2024-10-25 07:13:44 】ConstructBaseOption1 pBattleGameOption->BattleGameOption.cbEmployScene:100,cbEmployScene:100,wModuleID:65535,wKindID:98
			...
			(wModuleID == INVALID_WORD || wModuleID == m_BattleBaseOptionActive[i].BattleOption.wKindID))
		{
			...
			CString s;
			s.Format(_T("ConstructBaseOption2 pBattleGameOption->BattleGameOption.cbEmployScene:%d,cbEmployScene:%d,wModuleID:%d,wKindID:%d"), m_BattleBaseOptionActive[i].BattleOption.cbEmployScene, cbEmployScene, wModuleID, m_BattleBaseOptionActive[i].BattleOption.wKindID);
			...
			s.Format(_T("ConstructBaseOption3 pBattleGameOption->BattleGameOption.cbEmployScene:%d,cbEmployScene:%d,wModuleID:%d,wKindID:%d"), pBattleBaseOption->cbEmployScene, cbEmployScene, wModuleID, pBattleBaseOption->wKindID);
			CTraceService::TraceString(s, TraceLevel_Debug);
...
bool CAttemperEngineSink::OnTCPNetWorkMainBCBattleService(WORD wSubCmdID, VOID* pData, WORD wDataSize, DWORD dwSocketID, DWORD dwTokenID)
{
	...
	case SUB_BC_C_QUERY_OPTION:		//查询配置
	{
		...
		{	
			//aaap1_1
			/*CString s;
			s.Format(_T("ssssssssssssssssssssssssssss,wKindID:%d,cbEmployScene:%d"), pQueryOption->wKindID, pQueryOption->cbEmployScene);
			CTraceService::TraceString(s, TraceLevel_Debug);*/
			...
			if (wPacketSize > 0)
			{
				//发送数据
				/*CString s;
				s.Format(_T("ssssssssssssssssssssssssssss"));
				CTraceService::TraceString(s, TraceLevel_Debug); */
...
bool CAttemperEngineSink::OnDBBattleGameOption(DWORD dwContextID, DWORD dwTokenID, VOID * pData, WORD wDataSize)
{
	...
	for (int i = 0; i < wOptionCount; i++)
	{
		if (pBattleGameOption->BattleGameOption.wKindID == 98)
		{
			CString s;
			s.Format(_T("111 pBattleGameOption->BattleGameOption.wKindID:%d,cbEmployScene:%d"), pBattleGameOption->BattleGameOption.wKindID, pBattleGameOption->BattleGameOption.cbEmployScene);
			CTraceService::TraceString(s, TraceLevel_Debug);
		}
