...
BEGIN_MESSAGE_MAP(CLoggerServerDlg, CDialog)
...
//构造函数
CLoggerServerDlg::CLoggerServerDlg() : CDialog(IDD_DLG_LOGGER_SERVER)
{
	m_pServiceUnits = NULL;
}

//析构函数
CLoggerServerDlg::~CLoggerServerDlg()
{
	if (m_pServiceUnits) SafeDelete(m_pServiceUnits);
}

//控件绑定
VOID CLoggerServerDlg::DoDataExchange(CDataExchange * pDX)
{
...
//初始化函数
BOOL CLoggerServerDlg::OnInitDialog()
{
...
//确定消息
VOID CLoggerServerDlg::OnOK()
{
	return;
}

//取消函数
VOID CLoggerServerDlg::OnCancel()
{
...
//消息解释
BOOL CLoggerServerDlg::PreTranslateMessage(MSG * pMsg)
{
	//按键过滤
...
//服务状态
VOID CLoggerServerDlg::OnServiceUnitsStatus(WORD ServiceStatus)
{
	//状态设置
...
//设置标题
VOID CLoggerServerDlg::SetWindowStatusText(LPCTSTR pszServiceStatus)
{
	//获取对象
	...
	_sntprintf_s(szWindowText,CountArray(szWindowText),TEXT("日志服务器[%d] -- [ %s ]"), pServiceOption->wServicePort,pszServiceStatus);
...
//启动服务
VOID CLoggerServerDlg::OnBnClickedStartService()
{
	//设置接口
...
//停止服务
VOID CLoggerServerDlg::OnBnClickedConcludeService()
{
	//停止服务
	try
	{
...
//配置服务
VOID CLoggerServerDlg::OnBnClickedOptionService()
{	
	//设置资源
	...
	DlgServiceOption.SetDataFileName(TEXT("Logger.Data"));
...
//关闭询问
BOOL CLoggerServerDlg::OnQueryEndSession()
{
	//提示消息
	if (m_pServiceUnits->GetServiceStatus()!=SERVICE_STATUS_STOP)
	{
...
//命令处理
LRESULT CLoggerServerDlg::OnMessageProcessCmdLine(WPARAM wParam, LPARAM lParam)
{
