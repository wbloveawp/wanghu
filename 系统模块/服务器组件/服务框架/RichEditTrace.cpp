...
//接口查询
VOID * CRichEditTrace::QueryInterface(const IID & Guid, DWORD dwQueryVer)
{
...
//追踪信息
bool CRichEditTrace::TraceString(LPCTSTR pszString, enTraceLevel TraceLevel)
{
	//状态判断
	...
	_sntprintf_s(TraceInfo.szSring,CountArray(TraceInfo.szSring),TEXT("%s\n"),pszString);
...
bool CRichEditTrace::LoadMessage(LPCTSTR pszFileName)
{
	...
	EditStream.dwCookie=(DWORD)(SCORE)(&File);
...
//加载回调
DWORD CALLBACK CRichEditTrace::LoadCallBack(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb, LONG * pcb)
{
	//读取文件
	*pcb=((CFile *)(SCORE)(dwCookie))->Read(pbBuff,cb);

	return 0;
}

//保存回调
DWORD CALLBACK CRichEditTrace::SaveCallBack(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb, LONG * pcb)
{
	//写入文件
	((CFile *)(SCORE)(dwCookie))->Write(pbBuff,*pcb);
...
LRESULT CRichEditTrace::OnTraceServiceMessage(WPARAM wParam, LPARAM lParam)
{
	...
	CThreadLock ThreadLock(m_CriticalSection);
	m_DataQueue.DistillData(DataHead,m_cbBuffer,sizeof(m_cbBuffer),true);

	//效验数据
	ASSERT(DataHead.dwIdentifier==IDT_TRACE_STRING);
	if (DataHead.dwIdentifier!=IDT_TRACE_STRING) return 0;
	...
	_sntprintf_s(szTimeBuffer,CountArray(szTimeBuffer),TEXT("【 %04d-%02d-%02d %02d:%02d:%02d 】"),
