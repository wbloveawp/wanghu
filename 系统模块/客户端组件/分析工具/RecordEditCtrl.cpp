...
BEGIN_MESSAGE_MAP(CRecordEditCtrl, CRichEditCtrl)
...
//构造函数
CRecordEditCtrl::CRecordEditCtrl()
{
	//设置变量
...
//绑定函数
VOID CRecordEditCtrl::PreSubclassWindow()
{
...
//加载消息
bool CRecordEditCtrl::LoadMessage(LPCTSTR pszFileName)
{
	//效验参数
...
//保存消息
bool CRecordEditCtrl::SaveMessage(LPCTSTR pszFileName)
{
	//变量定义
...
//设置参数
bool CRecordEditCtrl::SetParameter(LONG lMaxLineCount, LONG lReserveLineCount)
{
	//效验参数
...
//配置服务
VOID CRecordEditCtrl::InitializeService()
{
	//清空队列
...
//插入字串
VOID CRecordEditCtrl::InsertString(LPCTSTR pszString, CHARFORMAT2 & CharFormat)
{
	//保存信息
	CHARRANGE CharRange;
	GetSel(CharRange);
...
//加载回调
DWORD CALLBACK CRecordEditCtrl::LoadCallBack(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb, LONG * pcb)
{
	//读取文件
...
//保存回调
DWORD CALLBACK CRecordEditCtrl::SaveCallBack(DWORD_PTR dwCookie, LPBYTE pbBuff, LONG cb, LONG * pcb)
{
	//写入文件
...
//删除信息
VOID CRecordEditCtrl::OnClearAll()
{
	//删除信息
	SetSel(0L, -1L);
	ReplaceSel(NULL);
...
//全部选择
VOID CRecordEditCtrl::OnSelectAll()
{
	//全部选择
	SetSel(0L, -1L);
...
//拷贝字符
VOID CRecordEditCtrl::OnCopyString()
{
	//拷贝字符
	Copy();
...
//保存信息
VOID CRecordEditCtrl::OnSaveString()
{
	//保存信息
	SaveMessage(NULL);
...
//删除字符
VOID CRecordEditCtrl::OnDeleteString()
{
	//删除字符
	ReplaceSel(TEXT(""));
...
//建立消息
INT CRecordEditCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
//右键消息
VOID CRecordEditCtrl::OnRButtonDown(UINT nFlags, CPoint Point)
{
	//设置焦点
	SetFocus();
