...
//读取事件
bool CMissionAndroid::OnEventMissionRead(CMD_Command Command, VOID * pData, WORD wDataSize)
{
	WORD wCmd = LOCMMD(Command.wMainCmdID);
	if (wCmd ==MDM_GP_ANDROID_SERVICE)
	{
		...
		case SUB_GP_ANDROID_PARAMETER:		//机器参数
			{
				//参数校验
				ASSERT(wDataSize<=sizeof(CMD_GP_RobotParameter));
				if(wDataSize>sizeof(CMD_GP_RobotParameter)) return false;

				//提取变量
				CMD_GP_RobotParameter* pAndroidParameter = (CMD_GP_RobotParameter*)pData;
				...
				{
					m_pIAndroidOperateCenter->OnEventEnableControls(TRUE);
					m_pIAndroidOperateCenter->OnEventAndroidParenter(pAndroidParameter->wSubCommdID, pAndroidParameter->wParameterCount, (pAndroidParameter->RobotParameter));
				}
		...
		switch (Command.wSubCmdID)
		{
			case SUB_GG_S_LINK_SUCCESS:		//接入服务器
			{
				...
				if (m_AndroidAction == AAction_GetParameter)
				{
					...
					GetMissionManager()->SendData(MAKECMMD(MDM_GP_ANDROID_SERVICE, SERVICE_MODULE_LOGON), SUB_GP_GET_PARAMETER, m_cbDataBuffer, m_wDataSize);
				}
				...
				if (m_AndroidAction == AAction_AddParameter)
				{
					//发送数据
					ASSERT(GetMissionManager() != NULL);
					GetMissionManager()->SendData(MAKECMMD(MDM_GP_ANDROID_SERVICE, SERVICE_MODULE_LOGON), SUB_GP_ADD_PARAMETER, m_cbDataBuffer, m_wDataSize);
				}
				...
				if (m_AndroidAction == AAction_ModifyParameter)
				{
					//发送数据
					ASSERT(GetMissionManager() != NULL);
					GetMissionManager()->SendData(MAKECMMD(MDM_GP_ANDROID_SERVICE, SERVICE_MODULE_LOGON), SUB_GP_MODIFY_PARAMETER, m_cbDataBuffer, m_wDataSize);
				}
				...
				if (m_AndroidAction == AAction_DeleteParameter)
				{
					//发送数据
					ASSERT(GetMissionManager() != NULL);
					GetMissionManager()->SendData(MAKECMMD(MDM_GP_ANDROID_SERVICE, SERVICE_MODULE_LOGON), SUB_GP_DELETE_PARAMETER, m_cbDataBuffer, m_wDataSize);
				}
...
//修改参数
VOID CMissionAndroid::ModifyAndroidParameter(WORD wServerID,tagRobotParameter * pAndroidParameter)
{
	//获取对象
	...
	CopyMemory(&pModifyParameter->RobotParameter,pAndroidParameter,sizeof(tagRobotParameter));
...
//添加参数
VOID CMissionAndroid::AddAndroidParameter(WORD wServerID,tagRobotParameter* pAndroidParameter)
{
	//获取对象
	...
	CopyMemory(&pAddParameter->RobotParameter,pAndroidParameter,sizeof(tagRobotParameter));
