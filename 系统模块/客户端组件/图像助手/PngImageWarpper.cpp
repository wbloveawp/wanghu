#include "StdAfx.h"
#include "PngImageWarpper.h"

//////////////////////////////////////////////////////////////////////////

//构造函数
CPngImageWarpper::CPngImageWarpper()
{
	//设置变量
	m_pImage=NULL;
...
//获取高度
INT CPngImageWarpper::GetHeight()
{
	//加载判断
	ASSERT(IsNull()==false);
	if (IsNull()==true) return 0;
...
//销毁图片
bool CPngImageWarpper::DestroyImage()
{
	//删除对象
...
//加载图片
bool CPngImageWarpper::LoadImage(LPCTSTR pszFileName)
{
	//加载判断
...
//加载图片
bool CPngImageWarpper::LoadImage(HINSTANCE hInstance, LPCTSTR pszResourceName)
{
	//加载判断
	ASSERT(m_pImage==NULL);
	if (m_pImage!=NULL) return false;
...
//绘画图像
bool CPngImageWarpper::DrawImage(CDC * pDC, INT nXPos, INT nYPos)
{
	//加载判断
	ASSERT(m_pImage!=NULL);
	if (m_pImage==NULL) return false;
...
//绘画图像
bool CPngImageWarpper::DrawImage(CDC * pDC, INT nXDest, INT nYDest, INT nDestWidth, INT nDestHeight, INT nXScr, INT YSrc)
{
	//加载判断
	ASSERT(m_pImage!=NULL);
	if (m_pImage==NULL) return false;
...
//绘画图像
bool CPngImageWarpper::DrawImage(CDC * pDC, INT nXDest, INT nYDest, INT nDestWidth, INT nDestHeight, INT nXScr, INT YSrc, INT nSrcWidth, INT nSrcHeight)
{
	//加载判断
	ASSERT(m_pImage!=NULL);
	if (m_pImage==NULL) return false;
