...
bool CExpression::GetExpressionPath(LPTSTR pszImagePath, WORD wMaxCount)
{
	...
	//构造地址
	_sntprintf_s(pszImagePath,wMaxCount, wMaxCount,TEXT("%s\\Expression\\%03d.GIF"),szDirectory,m_wIndex);
...
bool CExpressionManager::LoadExpression()
{
	...
	_sntprintf_s(szInfoFile,CountArray(szInfoFile),TEXT("%s\\Expression\\Expression.INI"),szDirectory);
	...
	do
	{
		...
		{
			//读取信息
			_sntprintf_s(szItemName,CountArray(szItemName),TEXT("%s%03d"),pszItemKey[i],wIndex);
...
VOID CExpressionControl::ShowExpression(CWnd * pParentWnd, INT nXPos, INT nYPos, IExpressionSink * pIExpressionSink)
{
	...
	SetWindowPos(NULL,nXPos,nYPos-nControlHeight,nControlWidth,nControlHeight,SWP_NOZORDER|SWP_SHOWWINDOW|SWP_NOZORDER);
...
BOOL CExpressionControl::OnSetCursor(CWnd * pWnd, UINT nHitTest, UINT uMessage)
{
	...
	if (m_wSelectIndex!=INVALID_WORD)
	{
		SetCursor(LoadCursor(GetModuleHandle(CLIENT_CONTROLS_DLL_NAME),MAKEINTRESOURCE(IDC_HAND_CUR)));
		return TRUE;
	}
