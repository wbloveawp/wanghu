...
#define D_EXP						0.000001							//积分精度

inline bool D_Equals(double dVal1, double dVal2)//等于
{
	if (fabs(dVal1 - dVal2) < D_EXP)
		return true;

	return false;
}
inline bool D_UnEquals(double dVal1, double dVal2)//不等于
{
	return !D_Equals(dVal1, dVal2);
}

inline bool D_LessThen(double dVal1, double dVal2)//小于
{
	if (dVal1 + D_EXP < dVal2)
		return true;
	return false;
}

inline bool D_UnLessThen(double dVal1, double dVal2)//大于等于
{
	return !D_LessThen(dVal1, dVal2);
}

inline bool D_GreaterThen(double dVal1, double dVal2)//大于
{
	if (dVal1 - D_EXP > dVal2)
		return true;
	return false;
...
inline bool D_UnGreaterThen(double dVal1, double dVal2)//小于等于
{
	return !D_GreaterThen(dVal1, dVal2);
}

inline LONGLONG D_Mod(double dVal1, double dVal2)//取模
{
	double dRes = dVal1 / dVal2;
	return (LONGLONG)(dRes + D_EXP);
}

inline double D_REM(double dVal1, double dVal2)//取余
{
	LONGLONG lMod = D_Mod(dVal1, dVal2);
	return dVal1 - lMod * dVal2;
}

inline double D_MIN(double dVal1, double dVal2)//取小
{
	if (D_LessThen(dVal1, dVal2))
		return dVal1;
	else
		return dVal2;
}

inline double D_MAX(double dVal1, double dVal2)//取大
{
	if (D_LessThen(dVal1, dVal2))
		return dVal2;
	else
