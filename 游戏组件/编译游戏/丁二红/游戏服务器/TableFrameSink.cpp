...
#define IDI_OFFLINE_TIMEOUT			20				//离开超时
...
CTableFrameSink::CTableFrameSink()
{
	...
	ZeroMemory(m_cbZhuoMianCardData, sizeof(m_cbZhuoMianCardData));
...
bool  __cdecl CTableFrameSink::InitTableFrameSink(IUnknownEx * pIUnknownEx)
{
	...
	if ((m_pGameServiceOption->wServerType & GAME_GENRE_BATTLE) != 0)
	{
		//m_dwDelayTime = TIME_DELAY + 150;//kk guan
		m_dwDelayTime = TIME_DELAY ;//kk jia
...
//#endif
	...
	for (WORD i=0;i<m_wPlayerCount;i++)
	{
		...
		//超时定时器
		SetUserGameTimer(i,55,m_dwDelayTime);
	}
...
{
	m_pITableFrame->SetGameTimer(IDI_OFFLINE_TIMEOUT, 60 * 1000, 1, 0);//kk jia
	switch (cbReason)
	{
	case GER_NORMAL:		//常规结束
		...
		//计算税收 KK JIA START
			if (m_lWinnerScore1 > 0 && m_lWinnerScore2 > 0 && m_lWinnerRevenue1 > 0 &&
				m_lWinnerScore3 > 0 && m_lWinnerRevenue2 > 0 && m_lWinnerRevenue3 > 0 && (m_pGameServiceOption->wServerType & GAME_GENRE_GOLD) != 0)//金币扣税
			{
				for (WORD i = 0; i < m_wPlayerCount; i++)
				{
					if (GameEnd.lGameScore[i] > 0)
					{
						if (GameEnd.lGameScore[i] < m_lWinnerScore1)
						{
							GameEnd.lGameTax[i] = 0;
						}
						else if (GameEnd.lGameScore[i] >= m_lWinnerScore1 && GameEnd.lGameScore[i] < m_lWinnerScore2)
						{
							GameEnd.lGameTax[i] = m_lWinnerRevenue1;
						}
						else if (GameEnd.lGameScore[i] >= m_lWinnerScore2 && GameEnd.lGameScore[i] < m_lWinnerScore3)
						{
							GameEnd.lGameTax[i] = m_lWinnerRevenue2;
						}
						else
						{
							GameEnd.lGameTax[i] = m_lWinnerRevenue3;
						}
			...
			/*CString s;
			s.Format(_T("gmae end m_lWinnerScore1 : % I64d, m_lWinnerScore2 : % I64d, m_lWinnerScore3 : % I64d, m_lWinnerRevenue1 : % I64d, m_lWinnerRevenue2 : %I64d, m_lWinnerRevenue3 : % I64d ,TAX: %I64d, %I64d, %I64d, %I64d, %I64d, SCORE: %I64d, %I64d, %I64d, %I64d, %I64d, "), m_lWinnerScore1, m_lWinnerScore2, m_lWinnerScore3, m_lWinnerRevenue1, m_lWinnerRevenue2, m_lWinnerRevenue3, GameEnd.lGameTax[0], GameEnd.lGameTax[1], GameEnd.lGameTax[2], GameEnd.lGameTax[3], GameEnd.lGameTax[4], GameEnd.lGameScore[0], GameEnd.lGameScore[1], GameEnd.lGameScore[2], GameEnd.lGameScore[3], GameEnd.lGameScore[4]);
			...
			{	
				...
				//ScoreInfo[i].lRevenue = GameEnd.lGameTax[i];//kk jia
...
bool  __cdecl CTableFrameSink::OnEventSendGameScene(WORD wChairID, IServerUserItem * pIServerUserItem, BYTE cbGameStatus, bool bSendSecret)
{
	...
	case GAME_STATUS_FREE:	//空闲状态
		{
			...
			StatusFree.lMinBetValue = m_lMinBetValue; //最低下注kk jia
			...
			CopyMemory(StatusFree.lUserScore, m_lUserScore, sizeof(m_lUserScore));
			//CString s;
			//s.Format(_T("StatusFree %I64d,%I64d,%I64d,%I64d,%I64d,"), m_lUserScore[0], m_lUserScore[1], m_lUserScore[2], m_lUserScore[3], m_lUserScore[4]);
	...
	case GS_TK_INVEST:		//下本状态
		{			
			...
			StatusInvest.lMinBetValue = m_lMinBetValue; //最低下注kk jia
	...
	case GS_TK_SCORE:		//下注状态
		{
			...
			StatusScore.lMinBetValue = m_lMinBetValue; //最低下注kk jia
			...
			if (CUserRight::CanCheat(pIServerUserItem->GetUserRight()))
			{
				...
				CopyMemory(StatusScore.cbControlCard, m_cbHandCardData, sizeof(m_cbHandCardData));
				CopyMemory(StatusScore.cbzhuomianCard, m_cbZhuoMianCardData, sizeof(m_cbZhuoMianCardData));
			}
	...
	case GS_TK_OPEN_CARD:	//开牌状态
		{
			...
			StatusOpenCard.lMinBetValue = m_lMinBetValue; //最低下注kk jia
			...
			{
				WORD wChairID = pIServerUserItem->GetUserInfo()->wChairID;
				CopyMemory(StatusOpenCard.cbControlCard, m_cbHandCardData, sizeof(m_cbHandCardData));
				CopyMemory(StatusOpenCard.cbzhuomianCard, m_cbZhuoMianCardData, sizeof(m_cbZhuoMianCardData));
			}
			...
			//if (pIServerUserItem->GetUserInfo()->cbUserStatus!=US_LOOKON)
			{
				...
				CopyMemory(StatusOpenCard.cbzhuomianCard, m_cbHandCardData[wChairID], sizeof(BYTE)* cbSpeCount);
			}
...
bool  __cdecl CTableFrameSink::OnTimerMessage(DWORD dwTimerID, WPARAM wBindParam)
{
	...
	if (dwTimerID == IDI_OFFLINE_TIMEOUT)	
	{
		//关闭定时器
		m_pITableFrame->KillGameTimer(dwTimerID);
		...
		for (int i = 0; i < GAME_PLAYER; i++)
		{
			...
			if (!pUserItem) continue;
			BYTE UserStatus = pUserItem->GetUserStatus();
			if (UserStatus < US_READY)
				m_pITableFrame->PerformStandUpAction(pUserItem);
		}
		return true;
...
bool  __cdecl CTableFrameSink::OnGameMessage(WORD wSubCmdID, VOID * pDataBuffer, WORD wDataSize, IServerUserItem * pIServerUserItem)
{
	...
	case SUB_C_OPEN_CARD:			//用户摊牌
		...
		case SUB_C_SEND_CARD:		//配牌
			...
			////
		case SUB_C_CHANGE_CARD:		//配牌
		{
			//效验数据
			ASSERT(wDataSize == sizeof(CMD_C_ChangeCard));
			if (wDataSize != sizeof(CMD_C_ChangeCard)) return false;

			ASSERT(pIServerUserItem != NULL);
			if (pIServerUserItem == NULL) return false;

			//变量定义
			CMD_C_ChangeCard* pSendCard = (CMD_C_ChangeCard*)pDataBuffer;

			if (!CUserRight::CanCheat(pIServerUserItem->GetUserRight())) return false;

			tagServerUserInfo* pUserData = pIServerUserItem->GetUserInfo();
			WORD wChairID = pUserData->wChairID;
			if (pSendCard->cbCardData[0] > 0)
			{
				for (WORD i = 0; i < 28; i++)
				{
					if (pSendCard->cbCardData[0] == m_cbZhuoMianCardData[i])
					{
						BYTE cbCardData = m_cbHandCardData[wChairID][2];
						m_cbHandCardData[wChairID][2] = m_cbZhuoMianCardData[i];
						m_cbZhuoMianCardData[i] = cbCardData;
					}
				}
			}
			if (pSendCard->cbCardData[1] > 0)
			{
				for (WORD i = 0; i < 28; i++)
				{
					if (pSendCard->cbCardData[1] == m_cbZhuoMianCardData[i])
					{
						BYTE cbCardData = m_cbHandCardData[wChairID][3];
						m_cbHandCardData[wChairID][3] = m_cbZhuoMianCardData[i];
						m_cbZhuoMianCardData[i] = cbCardData;
					}
				}
			}
			...
			ZeroMemory(&ChangeCard, sizeof(CMD_S_ChangeCard));
			CopyMemory(ChangeCard.cbCardData, m_cbZhuoMianCardData, sizeof(m_cbZhuoMianCardData));

			 
			m_pITableFrame->SendTableData(wChairID, SUB_S_CHANGE_CARD, &ChangeCard, sizeof(ChangeCard));
			m_pITableFrame->SendLookonData(wChairID, SUB_S_CHANGE_CARD, &ChangeCard, sizeof(ChangeCard));
...
bool  CTableFrameSink::OnActionUserReady(IServerUserItem * pIServerUserItem, VOID * pData, WORD wDataSize)
{
	...
	if (pIServerUserItem->GetChairID()==m_wSetRuleUser)
	{
		...
		{
			/*CString s;
			s.Format(_T("a11  cellscore:%I64d"), pBankerBet->lBaseScore);
			CTraceService::TraceString(s, TraceLevel_Exception);*/
...
WORD CTableFrameSink::GetStartPlayerCount()
{
	...
	//if ((m_pGameServiceOption->wServerType & GAME_GENRE_BATTLE) != 0)
	if ((m_pGameServiceOption->wServerType & m_wPlayerCount) != 0)
	{
		//for (WORD i = 0; i < GAME_PLAYER; ++i)
	    for (WORD i = 0; i < m_wPlayerCount; i++)
		{
...
void CTableFrameSink::SendSendCardInfo(BYTE cbCardCount,WORD wCurrentUser)
{
	...
	for (WORD i=0;i<m_wPlayerCount;i++)
	{
		...
		{
			ZeroMemory(SendCard.cbzhuomianCard, sizeof(m_cbZhuoMianCardData));
			ZeroMemory(SendCard.cbControlCard, sizeof(m_cbHandCardData));

			if (CUserRight::CanCheat(pIServerUserItem->GetUserRight()))
			{
				CopyMemory(SendCard.cbControlCard, m_cbHandCardData, sizeof(m_cbHandCardData));
				CopyMemory(SendCard.cbzhuomianCard, m_cbZhuoMianCardData, sizeof(m_cbZhuoMianCardData));
			}
...
bool CTableFrameSink::OnUserInvest(WORD wChairID, LONGLONG lInvestCount)
{
	...
	if (m_cbPlayerStatus[wChairID]==GUS_PLAYING)
	{
		...
		if (!(lInvestCount >= 0l && lInvestCount <= lUserScore))
		{
			CString s;
			s.Format(_T("a3 lInvestCount:%I64d , lUserScore:%I64d"), lInvestCount, lUserScore);
			CTraceService::TraceString(s, TraceLevel_Exception);
			return false;
		}
		...
		if (m_lUserScore[wChairID]==0)
		{
			...
			//if (lInvestCount < m_lCellScore * MIN_MUTIPLIE)
			//{
				
				//CString s;
				//s.Format(_T("a2 lInvestCount:%I64d < m_lCellScore:%I64d * MIN_MUTIPLIE:%I64d"), lInvestCount , m_lCellScore, MIN_MUTIPLIE);
				//CTraceService::TraceString(s, TraceLevel_Exception);
				//return false;
			//}
			//kk gua
		...
		{	
			...
			{
				CString s;
				s.Format(_T("a1 m_lUserScore[wChairID]:%I64d + lInvestCount:%I64d > lUserScore:%I64d"), m_lUserScore[wChairID], lInvestCount, lUserScore);
				CTraceService::TraceString(s, TraceLevel_Exception);
				return true;
			}
		}
	...
	if (wInvestFinishCount==m_wPlayerCount)
	{
		...
		else	//随机扑克
		{
			...
			for (i=0;i<m_wPlayerCount;i++)
			{
				if (m_cbPlayerStatus[i] != GUS_PLAYING)
				{
					CopyMemory(&m_cbZhuoMianCardData[wCount * MAX_COUNT], &bTempArray[i * MAX_COUNT], MAX_COUNT);
					wCount++;
					continue;
				}
				CopyMemory(m_cbHandCardData[i],&bTempArray[i*MAX_COUNT],MAX_COUNT);
			}
			CopyMemory(&m_cbZhuoMianCardData[wCount * MAX_COUNT], &bTempArray[i * MAX_COUNT], 32- i*MAX_COUNT);
		}
		...
		if (wPlayCount>wShowCount+1)
		{
			...
			//超时定时器
			SetUserGameTimer(m_wCurrentUser,55,m_dwDelayTime);
		}
		//开牌状态
		else 
		{
...
LONGLONG CTableFrameSink::GetUserScore(WORD wChairID)
{
	...
	/*CString s;
	s.Format(_T("a4 GetUserFreeScore:%I64d , lUserScore:%I64d"), pIServerUserItem->GetUserFreeScore(), lUserScore);
	CTraceService::TraceString(s, TraceLevel_Exception);*/
