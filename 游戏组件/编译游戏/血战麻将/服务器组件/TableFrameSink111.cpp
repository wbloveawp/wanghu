...
bool CTableFrameSink::OnEventGameStart()
{
	...
	strSaveFileName.Format(TEXT("%s\\ConfigCard\\GameCardData\\SparrowXZ.card"), strPath);
	CFile file;
	...
	case GER_NORMAL:		//常规结束
		{
			...
			for( WORD i = 0; i < GAME_PLAYER; i++ )
			{
				...
				if (m_lGameScore[i] > 10000000)
				{
                    ...
                    strOperationRecord.Format(TEXT("table id is %d, 操作时间: %d/%d/%d-%d:%d:%d, 账户[%s],得分为 %.2f"), m_pITableFrame->GetTableID(),
		...
		if (cbTargetAction == WIK_CHI_HU)
		{
			...
			for (WORD i=(m_wProvideUser+GAME_PLAYER-1)%GAME_PLAYER; i!=m_wProvideUser; i = (i+GAME_PLAYER-1)%GAME_PLAYER)
			{
				...
				if (m_dwChiHuKind[i] != WIK_NULL) 
				{
					...
					CopyMemory(ChiHu.lHuScore, lChiHuScore, sizeof(lChiHuScore));
		...
		case WIK_CHI_HU:		//吃胡操作
			{
				...
				lChiHuScore[wChairID] = lZiMoChiHuScore;
				...
				for (WORD i=0; i<GAME_PLAYER; i++)
				{
					...
					lChiHuScore[i] = -(lZiMoChiHuScore / cbValidCount);
				}
