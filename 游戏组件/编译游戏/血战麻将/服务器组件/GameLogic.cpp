...
//听牌分析
bool CGameLogic::AnalyseTingCard(const BYTE cbCardIndex[MAX_INDEX], const tagWeaveItem WeaveItem[], BYTE cbWeaveCount, BYTE cbHuaZhuCard)
{
	//复制数据
	BYTE cbCardIndexTemp[MAX_INDEX];
	ZeroMemory(cbCardIndexTemp, sizeof(cbCardIndexTemp));
...
BYTE CGameLogic::GetTingDataEx(const BYTE cbCardIndex[MAX_INDEX], const tagWeaveItem WeaveItem[], BYTE cbWeaveCount, BYTE& cbOutCardCount, BYTE cbOutCardData[], BYTE cbHuCardCount[], BYTE cbHuCardData[][HEAP_FULL_COUNT], BYTE cbHuFan[][HEAP_FULL_COUNT], BYTE cbHuaZhuCard)
{
	//复制数据
	BYTE cbOutCount = 0;
	BYTE cbCardIndexTemp[MAX_INDEX];
		...
		for (BYTE i = 0; i < MAX_INDEX; i++)
		{
			...
			for (BYTE j = 0; j < MAX_INDEX; j++)
			{
				...
				if (WIK_CHI_HU == AnalyseChiHuCard(cbCardIndexTemp, WeaveItem, cbWeaveCount, cbCurrentCard, chr, cbHuaZhuCard))
				{
					...
					ASSERT(cbHuFan[cbOutCount - 1][nCount] != INVALID_BYTE);
...
//获取吃胡番数
WORD CGameLogic::GetUserHuFan(const CChiHuRight & ChiHuRight)
{
	//普通番
	WORD wFanShu = 0;
