...
-- 游戏记录
CREATE PROC [dbo].[GSP_GR_RecordDrawInfo]
...
BEGIN
	...
	BEGIN
		INSERT RecordDrawInfo (RecordID, KindID, ServerID, TableID, UserCount, RevenueCount, WasteCount, PlayTimeCount, VideoNumber, MatchNo, RecordDate)
		VALUES (@lRecordID, @wKindID, @wServerID, @wTableID, @wUserCount, @lRevenueCount, @lWasteCount, @dwPlayTimeCount, @strVideoNumber, @lRelationMatchNo, GetDate())
	END
...
BEGIN
	...
	BEGIN
		INSERT INTO RecordDrawScore(RecordID,UserID,ChairID,Score,Revenue,VideoNumber,MatchNo,RecordDate)
		SELECT @lRecordID,JSON_VALUE(VALUE,N'$.UserID'),JSON_VALUE(VALUE,N'$.ChairID'),JSON_VALUE(VALUE,N'$.Score'),JSON_VALUE(VALUE,N'$.Revenue'),@strVideoNumber,@lRelationMatchNo,GETDATE()
