...
BEGIN
    ...
    BEGIN
		...
		-- 系统损耗
		UPDATE GameScoreInfo SET WasteCount = WasteCount+@lWasteCount WHERE UserID = @dwWasterUserID		

		-- 更新损耗
		UPDATE StreamWasteInfo SET WasteScoreStat=WasteScoreStat+@lWasteCount,LastWasteDate=GETDATE() WHERE DateID=@DateID AND TypeID=@Type
		IF @@ROWCOUNT=0
		BEGIN
			INSERT INTO StreamWasteInfo(DateID,TypeID,WasteScoreStat,FirstWasteDate,LastWasteDate) VALUES(@DateID, @Type, @lWasteCount, GETDATE(), GETDATE())
		END
    END
END
...
BEGIN
	...
	BEGIN
		INSERT INTO RecordDrawScore(RecordID,UserID,ChairID,Score,Revenue,BeforeScore,VideoNumber,RecordDate)
		SELECT @lRecordID,JSON_VALUE(VALUE,N'$.UserID'),JSON_VALUE(VALUE,N'$.ChairID'),JSON_VALUE(VALUE,N'$.Score'),JSON_VALUE(VALUE,N'$.Revenue'),JSON_VALUE(VALUE,N'$.BeforeScore'),@strVideoNumber,GETDATE()
