...
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[GSP_MB_RegisterEMail]') and OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[GSP_MB_RegisterEMail]
GO
...
-- 邮箱注册
CREATE PROC [dbo].[GSP_MB_RegisterEMail]
	@wGateID INT,								-- 网关标识	
	@wLogonID INT,								-- 登录标识
	@dwSocketID INT,							-- 网络标识
...
BEGIN
	...
	-- 查询用户
	IF EXISTS (SELECT 1 FROM AccountsInfo(NOLOCK) WHERE Accounts=@strEMail)
	BEGIN
		SET @strErrorDescribe=N'{"Idx":1015,"Msg":"此邮箱已被注册，请换另一邮箱尝试再次注册！"}'
		RETURN 7
	END
	...
	-- 查询邮箱
	IF EXISTS (SELECT 1 FROM AccountsInfo(NOLOCK) WHERE EMail=@strEMail)
	BEGIN
		SET @strErrorDescribe=N'{"Idx":1016,"Msg":"此邮箱已被其他游戏账号绑定，请换另一邮箱尝试再次注册！"}'
		RETURN 7
	END	

	-- 生成账号
	SET @Accounts=CONCAT(N'EMail',SUBSTRING(@strEMail,1,5))
	SET @strNickName=CONCAT(SUBSTRING(@strEMail,1,5),N':',N'EMail')	
	...
	INSERT AccountsInfo (Accounts,NickName,RegAccounts,LogonPass,InsurePass,Gender,FaceID,LogonMachineID, SecretKey, RegisterMode,RegisterMachineID,RegisterDeviceKind,RegisterPackageName,GameLogonTimes,
		  RegisterIP,LogonIP,LogonMode,PlazaVersion,MarketID,EMail)
	VALUES (@strEMail,@strNickName,@strEMail,@strPassword,N'',@cbGender,@wFaceID,@strMachineID,@SecretKey,2,@strMachineID,@dwDeviceKind,@strPackageName,1,
	     @strClientIP,@strClientIP,4,@dwPlazaVersion,@wMarketID,@strEMail)

	-- 错误判断
	IF @@ERROR<>0
	BEGIN
		SET @strErrorDescribe=N'{"Idx":1018,"Msg":"邮箱已存在，请换另一邮箱尝试再次注册！"}'
		RETURN 8
	END	
	...
	SELECT @UserID=UserID, @GameID=GameID, @Accounts=Accounts,@NickName=NickName,@FaceID=FaceID, @CustomID=CustomID, @ProtectID=ProtectID, @MasterOrder=MasterOrder, @Gender=Gender, @Experience=Experience, 
		@LoveLiness=LoveLiness, @UnderWrite=UnderWrite, @MemberPoint=MemberPoint, @MemberOverTime=MemberOverTime, @LikedNumber=LikedNumber,@InfoPublic=InfoPublic,@ActionMaskEver=ActionMaskEver,
		@UserRight=UserRight,@ActionMaskPerDay=ActionMaskPerDay, @ActionMaskPerWeek=ActionMaskPerWeek,@RegisterMode=RegisterMode,@RegisterTime=dbo.LHF_ConvertTimestamp(RegisterDate)
	FROM AccountsInfo(NOLOCK) WHERE Accounts=@strEMail
	...
	SELECT @UserID AS UserID, @GameID AS GameID, @Accounts AS Accounts, @NickName AS NickName, @strPassword AS Password, @InsurePass AS InsurePass, @FaceID AS FaceID, @CustomID AS CustomID, @ProtectID AS ProtectID,
		   @MasterOrder AS MasterOrder, @SecretKey AS SecretKey, @UserGold AS Gold, @InsureGold AS InsureGold, @UserCard AS UserCard, @UserGold AS UserGold, @Commission AS Commission, @Gender AS Gender, 
		   @LockGateID AS LockGateID, @LockLogonID AS LockLogonID, @LockSocketID AS LockSocketID, @LockServerID AS LockServerID, @LockKindID AS LockKindID, @LockServerKind AS LockServerKind, @Experience AS Experience,
		   @LoveLiness AS LoveLiness, @MemberPoint AS MemberPoint, @MemberOverTime AS MemberOverTime, @UnderWrite AS UnderWrite, N'' AS MobilePhone, @strEMail AS EMail, N'' AS UniqueID, @LikedNumber AS LikedNumber, 
		   @InfoPublic AS InfoPublic, @ActionMaskEver AS ActionMaskEver, @ActionMaskPerDay AS ActionMaskPerDay, @ActionMaskPerWeek AS ActionMaskPerWeek, @RegisterTime AS RegisterTime, 1 AS GameLogonTimes,
		   @ParentGameID AS ParentGameID,@ParentFaceID AS ParentFaceID,@ParentNickName AS ParentNickName, 0 AS ShutUpStatus, 0 AS ShutUpOverTime, @RegisterMode AS RegisterMode, @UIShieldMask AS UIShieldMask,
		   @UserRight AS UserRight,@ShieldKindList AS ShieldKindList ,@szCompellation AS Compellation,@szPassPortID AS PassPortID,@szPresentGoodsList AS PresentGoodsList,@CheckInDays AS CheckInDays,@CheckInState AS CheckInState,
