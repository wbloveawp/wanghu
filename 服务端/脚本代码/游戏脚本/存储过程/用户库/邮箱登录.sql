...
IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[GSP_MB_EfficacyEMail]') and OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[GSP_MB_EfficacyEMail]
GO

IF EXISTS (SELECT * FROM DBO.SYSOBJECTS WHERE ID = OBJECT_ID(N'[dbo].[GSP_MB_EfficacyEMailCode]') and OBJECTPROPERTY(ID, N'IsProcedure') = 1)
DROP PROCEDURE [dbo].[GSP_MB_EfficacyEMailCode]
GO
...
-- 邮箱登陆
CREATE PROC GSP_MB_EfficacyEMail
	@wGateID INT,								-- 网关标识	
	@wLogonID INT,								-- 登录标识
	@dwSocketID INT,							-- 网络标识
	@strEMail NVARCHAR(32),						-- 邮箱
...
BEGIN
	...
	SELECT @UserID=a.UserID, @GameID=a.GameID, @Accounts=a.Accounts, @NickName=a.NickName, @UnderWrite=a.UnderWrite, @LogonPass=a.LogonPass, @InsurePass=a.InsurePass, @FaceID=a.FaceID,@CustomID=a.CustomID,
		...
		@MemberOverTime=a.MemberOverTime, @MachineID=a.LogonMachineID, @PlazaVersion=a.PlazaVersion,@LikedNumber=a.LikedNumber, @EMail=EMail, @MobilePhone=MobilePhone, @RegisterMode=a.RegisterMode, @InfoPublic=a.InfoPublic,
		@RegisterDeviceKind=a.RegisterDeviceKind,@ActionMaskEver=a.ActionMaskEver,@ActionMaskPerDay=a.ActionMaskPerDay, @ActionMaskPerWeek=a.ActionMaskPerWeek, @ParentUserID=a.ParentID,@ShutUpStatus=b.ShutupStatus,
		@ShutDownStatus=b.ShutDownStatus,@ShutupOverTime=b.ShutupOverTime,@ShutDownOverTime=b.ShutDownOverTime, @LogonDateTime=a.LogonDate,@ResetDateTime=a.ResetDateTime,@RegisterTime=dbo.LHF_ConvertTimestamp(a.RegisterDate)
	FROM AccountsInfo(NOLOCK) a LEFT JOIN AccountsStatus(NOLOCK) b ON a.UserID=b.UserID WHERE a.EMail=@strEMail
	...
	-- 更新信息
	UPDATE AccountsInfo SET SecretKey=@SecretKey,GameLogonTimes=@GameLogonTimes,LogonDate=GETDATE(), LogonIP=@strClientIP,LogonMode=4,
	PlazaVersion=@dwPlazaVersion,LogonMachineID=@strMachineID,LogonEMail=@strEMail WHERE UserID=@UserID	
	...
	SELECT @UserID AS UserID, @GameID AS GameID, @Accounts AS Accounts, @NickName AS NickName, @LogonPass AS Password, @InsurePass AS InsurePass, @UnderWrite AS UnderWrite, @FaceID AS FaceID, @CustomID AS CustomID,
		...
		@LockSocketID AS LockSocketID,@LockServerID AS LockServerID, @EMail AS EMail, @MobilePhone AS MobilePhone, @UniqueID AS UniqueID, @RegisterTime AS RegisterTime, @LikedNumber AS LikedNumber, @ActionMaskEver AS ActionMaskEver,
		...
		@ParentNickName AS ParentNickName,@ShutUpStatus AS ShutUpStatus, @ShutUpOverTime AS ShutUpOverTime, @RegisterMode AS RegisterMode, @UIShieldMask AS UIShieldMask, @LogonDateTime AS LastLogonDate,
...
-- 验证码登陆
CREATE PROC GSP_MB_EfficacyEMailCode
	@wGateID INT,								-- 网关标识	
	@wLogonID INT,								-- 登录标识
	@dwSocketID INT,							-- 网络标识	
...
BEGIN
	...
	SELECT @UserID=a.UserID, @GameID=a.GameID, @Accounts=a.Accounts, @NickName=a.NickName, @UnderWrite=a.UnderWrite, @LogonPass=a.LogonPass, @InsurePass=a.InsurePass, @FaceID=a.FaceID,@CustomID=a.CustomID,
		@ProtectID=a.ProtectID,@MasterOrder=a.MasterOrder,@Gender=a.Gender, @Nullity=a.Nullity, @Experience=a.Experience, @LoveLiness=a.LoveLiness, @GameLogonTimes=a.GameLogonTimes, @MemberPoint=a.MemberPoint, 
		@MemberOverTime=a.MemberOverTime, @MachineID=a.LogonMachineID, @PlazaVersion=a.PlazaVersion,@LikedNumber=a.LikedNumber, @RegisterMode=a.RegisterMode, @InfoPublic=a.InfoPublic,@RegisterDeviceKind=a.RegisterDeviceKind,
		@ActionMaskEver=a.ActionMaskEver,@ActionMaskPerDay=a.ActionMaskPerDay,@ActionMaskPerWeek=a.ActionMaskPerWeek,@LogonDateTime=a.LogonDate,@ResetDateTime=a.ResetDateTime,@ParentUserID=a.ParentID,@ShutUpStatus=b.ShutupStatus,
		@UserRight=a.UserRight,@ShutDownStatus=b.ShutDownStatus,@ShutupOverTime=b.ShutupOverTime,@ShutDownOverTime=b.ShutDownOverTime, @MobilePhone=a.MobilePhone,@RegisterTime=dbo.LHF_ConvertTimestamp(a.RegisterDate)
	FROM AccountsInfo(NOLOCK) a LEFT JOIN AccountsStatus(NOLOCK) b ON a.UserID=b.UserID  WHERE a.EMail=@strEMail	
	...
	SELECT @ATAuthTimes=AuthTimes,@ATLastAcquireDate=LastAcquireDate FROM AccountsAuthInfo(NOLOCK) WHERE UserID=@UserID AND CodeKind=2

	-- 插入数据
	IF @ATAuthTimes IS NULL
	BEGIN
		INSERT INTO AccountsAuthInfo(UserID,CodeKind,AuthTimes,AcquireTimes,LastAcquireDate,LastMobileEmail)	VALUES (@UserID,2,1,0,GETDATE(),N'')		
	END ELSE
	BEGIN
		-- 时间间隔
		...
		-- 更新数据
		UPDATE AccountsAuthInfo SET AuthTimes=@ATAuthTimes+1,AcquireTimes=0,LastAcquireDate=GETDATE(),LastMobileEmail=N'' WHERE UserID=@UserID AND CodeKind=2
	END
	...
	-- 记录验证次数
	UPDATE RecordAuthTimes SET AuthTimes=AuthTimes+1 WHERE DateID=@DateID AND UserID=@UserID AND CodeKind=2 AND AuthKind=@AuthKind
	IF @@ROWCOUNT=0
	BEGIN
		-- 插入记录
		INSERT INTO RecordAuthTimes(DateID,UserID,CodeKind,AuthKind,AuthTimes) VALUES (@DateID,@UserID,2,@AuthKind,1)
	END
	...
	-- 输出变量
	SELECT @UserID AS UserID, @GameID AS GameID, @Accounts AS Accounts, @NickName AS NickName, @LogonPass AS Password, @InsurePass AS InsurePass, @UnderWrite AS UnderWrite,@SecretKey AS SecretKey,  
		   @FaceID AS FaceID, @CustomID AS CustomID,@ProtectID AS ProtectID,@MasterOrder AS MasterOrder, @UserGold AS UserGold, @InsureGold AS InsureGold, @UserCard AS UserCard, @Commission AS Commission, 
		   @Gender AS Gender, @Experience AS Experience, @LoveLiness AS LoveLiness, @MemberPoint AS MemberPoint, @MemberOverTime AS MemberOverTime, @LockGateID AS LockGateID, @InfoPublic AS InfoPublic,
		   @LockLogonID AS LockLogonID, @LockSocketID AS LockSocketID, @LockKindID AS LockKindID, @LockServerID AS LockServerID, @strEMail AS EMail, @MobilePhone AS MobilePhone, @UniqueID AS UniqueID, 
		   @RegisterTime AS RegisterTime, @ActionMaskEver AS ActionMaskEver, @ActionMaskPerDay AS ActionMaskPerDay, @ActionMaskPerWeek AS ActionMaskPerWeek, @LikedNumber AS LikedNumber, @GameLogonTimes AS GameLogonTimes,
		   @LockServerKind AS LockServerKind,@ParentGameID AS ParentGameID,@ParentFaceID AS ParentFaceID,@ParentNickName AS ParentNickName,@ShutUpStatus AS ShutUpStatus, @ShutUpOverTime AS ShutUpOverTime,
		   @UIShieldMask AS UIShieldMask, @LogonDateTime AS LastLogonDate,@szCompellation AS Compellation,@szPassPortID AS PassPortID,@CheckInDays AS CheckInDays,@CheckInState AS CheckInState,@UserRight AS UserRight,
