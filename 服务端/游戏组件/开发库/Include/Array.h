...
//自动缓冲
template <class TYPE, UINT uItemCount> class CWHBuffer
{
	//缓冲变量
protected:
	TYPE* m_pData;							//数组指针
	ULONG							m_uItemCount;						//缓冲数目

	//变量定义
protected:
	TYPE							m_DataArray[uItemCount];			//数组缓冲
...
public:
	...
	VOID DeleteBuffer();
	//获取缓冲
	TYPE* CreateBuffer(ULONG uItemCount);
...
public:
	...
	//获取数目
	ULONG GetItemCount() { return m_uItemCount; }
	//获取大小
	ULONG GetBufferSize() { return m_uItemCount * sizeof(TYPE); }
};
...
//构造函数
template <class TYPE, UINT uItemCount> CWHBuffer<TYPE, uItemCount>::CWHBuffer()
{
	//设置变量
	m_pData = m_DataArray;
	m_uItemCount = CountArray(m_DataArray);
...
//释放缓冲
template <class TYPE, UINT uItemCount> VOID CWHBuffer<TYPE, uItemCount>::DeleteBuffer()
{
	//删除缓冲
	if (m_pData != m_DataArray)
	{
...
//获取缓冲
template <class TYPE, UINT uItemCount> TYPE* CWHBuffer<TYPE, uItemCount>::CreateBuffer(ULONG uItemCount)
{
	//缓冲判断
	if (uItemCount <= m_uItemCount)
	{
		return m_pData;
	}
