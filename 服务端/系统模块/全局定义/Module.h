...
bool CTempldateHelper<IModeluInterface>::CreateInstance()
{
	...
	try
	{
		...
		{
			_sntprintf_s(m_szDescribe,CountArray(m_szDescribe),TEXT("“%s”模块加载失败,错误代码：%d。"),m_szModuleDllName,GetLastError());
			return false;
		}
		...
		if (CreateProc==NULL) 
		{
			_sntprintf_s(m_szDescribe,CountArray(m_szDescribe),TEXT("找不到组件创建函数“%hs”"),m_szCreateProc);
			return false;
		}
		...
		{
			_sntprintf_s(m_szDescribe,CountArray(m_szDescribe),TEXT("调用函数“%hs”生成对象失败"),m_szCreateProc);
			return false;
		}
	}
	...
	{ 
		_sntprintf_s(m_szDescribe,CountArray(m_szDescribe),TEXT("组件创建函数“%hs”产生未知异常错误，组件创建失败"),m_szCreateProc);
		return false;
	}
...
//组件辅助类宏
#define DECLARE_MODULE_DYNAMIC(OBJECT_NAME,INTERFACE_NAME)													\
class C##OBJECT_NAME##Helper : public CTempldateHelper<I##INTERFACE_NAME>									\
{																											\
public:																										\
	C##OBJECT_NAME##Helper() : CTempldateHelper<I##INTERFACE_NAME>(IID_I##INTERFACE_NAME,VER_I##INTERFACE_NAME) { }	\
};
...
//组件辅助类宏
#define DECLARE_MODULE_HELPER(OBJECT_NAME,INTERFACE_NAME,MODULE_DLL_NAME,CREATE_FUNCTION_NAME)				\
class C##OBJECT_NAME##Helper : public CTempldateHelper<I##INTERFACE_NAME>									\
{																											\
public:																										\
	C##OBJECT_NAME##Helper() : CTempldateHelper<I##INTERFACE_NAME>(IID_I##INTERFACE_NAME,					\
		VER_I##INTERFACE_NAME,MODULE_DLL_NAME,CREATE_FUNCTION_NAME) { }										\
};
