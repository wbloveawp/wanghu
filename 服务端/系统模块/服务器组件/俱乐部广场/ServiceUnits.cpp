...
CServiceUnits::CServiceUnits()
{
	...
	ZeroMemory(&m_LogicOption,sizeof(m_LogicOption));
...
//设置配置
void CServiceUnits::SetServiceOption(tagServiceOption &LogicOption)
{
	//拷贝配置
	CopyMemory(&m_LogicOption,&LogicOption,sizeof(m_LogicOption));
}
...
bool CServiceUnits::InitializeService()
{
	...
	//调度回调
	m_AttemperEngineSink.m_pLogicOption=&m_LogicOption;
	...
	//配置网络
	WORD wMaxConnect=m_LogicOption.wMaxConnect;
	WORD wServicePort=m_LogicOption.wServicePort;
	...
	case UI_CENTER_RESULT:		//中心成功
		{
			...
			{							
				//连接状态
				SendControlPacket(CT_LOAD_CLUB_OPTION,NULL,0);
			}

			return 0;
		}
	case UI_LOAD_CLUB_OPTION:		//加载配置
		{
			//效验消息
			ASSERT(DataHead.wDataSize==sizeof(CP_ControlResult));
			if (DataHead.wDataSize!=sizeof(CP_ControlResult)) return 0;
