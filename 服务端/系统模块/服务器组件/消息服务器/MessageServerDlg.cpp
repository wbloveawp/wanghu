...
//构造函数
CMessageServerDlg::CMessageServerDlg() : CDialog(IDD_DLG_MESSAGE_SERVER)
{
	//设置变量
...
//析构函数
CMessageServerDlg::~CMessageServerDlg()
{
...
//确定消息
VOID CMessageServerDlg::OnOK()
{
	return;
}

//取消函数
VOID CMessageServerDlg::OnCancel()
{
	if (m_pServiceUnits->GetServiceStatus()!=SERVICE_STATUS_STOP)
	{
		LPCTSTR pszQuestion=TEXT("消息服务器正在运行中，您确实要关闭服务器吗？");
...
//消息解释
BOOL CMessageServerDlg::PreTranslateMessage(MSG * pMsg)
{
	//按键过滤
...
//服务状态
VOID CMessageServerDlg::OnServiceUnitsStatus(WORD ServiceStatus)
{
	//状态设置
...
//设置标题
VOID CMessageServerDlg::SetWindowStatusText(LPCTSTR pszServiceStatus)
{
	//获取对象
	...
	_sntprintf_s(szWindowText,CountArray(szWindowText),TEXT("消息服务器[%d] -- [ %s ]"), pServiceOption->wServicePort,pszServiceStatus);
...
//启动服务
VOID CMessageServerDlg::OnBnClickedStartService()
{
	//设置接口
...
//停止服务
VOID CMessageServerDlg::OnBnClickedConcludeService()
{
	//停止服务
	try
	{
...
//配置服务
VOID CMessageServerDlg::OnBnClickedOptionService()
{	
	//设置资源
	...
	DlgServiceOption.SetDataFileName(TEXT("Message.Data"));
...
//关闭询问
BOOL CMessageServerDlg::OnQueryEndSession()
{
	//提示消息
	if (m_pServiceUnits->GetServiceStatus()!=SERVICE_STATUS_STOP)
	{
...
//命令处理
LRESULT CMessageServerDlg::OnMessageProcessCmdLine(WPARAM wParam, LPARAM lParam)
{
