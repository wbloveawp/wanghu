...
bool CServiceUnits::StartService()
{
	...
	//授权验证
	SendControlPacket(CT_LOAD_CONFIG_DATA,NULL,0);
...
{
	//拷贝配置
	CopyMemory(&m_ServiceOption, &ServiceOption, sizeof(m_ServiceOption));

	//设置参数
	m_MonitorClient.SetServiceParameter(SERVICE_MODULE_LOGON, m_ServiceOption.wServiceID, m_ServiceOption.szServiceName);
}
...
bool CServiceUnits::InitializeService()
{
	...
	if (m_DataBaseEngine->SetDataBaseEngineSink(pDataBaseEngineSink,CountArray(m_DataBaseEngineSink))==false) return false;
	...
	{
		m_DataBaseEngineSink[i].m_pServiceOption=&m_ServiceOption;
...
LRESULT CServiceUnits::OnUIControlRequest(WPARAM wParam, LPARAM lParam)
{
	...
	{
	case UI_LOAD_CONFIG_RESULT:	//列表成功
		{
			//效验消息
