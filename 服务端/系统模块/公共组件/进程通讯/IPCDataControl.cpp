...
//发送数据
bool CProcessSendData::SendData(WORD wMainCmdID, WORD wSubCmdID, VOID * const pData, WORD wDataSize)
{
	//效验参数
	ASSERT(IsWindow(m_hWndServer));
	if (IsWindow(m_hWndServer)==FALSE) return false;
...
//执行命令
bool CProcessSendData::TransferCommand(WORD wMainCmdID, WORD wSubCmdID, VOID * const pData, WORD wDataSize)
{
	//效验参数
	ASSERT(IsWindow(m_hWndServer));
	if (IsWindow(m_hWndServer)==FALSE) return false;
