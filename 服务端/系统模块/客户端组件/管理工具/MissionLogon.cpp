...
////请求缓冲
//DWORD __cdecl CMissionLogon::OnHttpRequestBuffer(DWORD dwRequestSize, LPVOID *ppvBuffer)
//{
//	if( ppvBuffer != NULL )
//	{
...
//
////下载完成
//bool __cdecl CMissionLogon::OnHttpDataComplete(DWORD dwDataSize)
//{
//	//变量定义
...
//
////下载异常
//bool __cdecl CMissionLogon::OnHttpDataException(enDownLoadError DownLoadError)
//{
//	//进度界面
//	CloseStatusInfo();
//
...
//
////连接关闭
//bool __cdecl CMissionLogon::OnHttpSocketClose(bool bCloseByServer)
//{
...
//读取事件
bool CMissionLogon::OnEventMissionRead(CMD_Command Command, VOID * pData, WORD wDataSize)
{
	...
	case MDM_GG_GATE_SERVICE:		//接入服务
		{
			if (Command.wSubCmdID == SUB_GG_S_LINK_SUCCESS)
			{
				...
				//发送数据
				CMissionManager* pMissionManager = GetMissionManager();
				pMissionManager->SendData(MAKECMMD(MDM_GP_LOGON, SERVICE_MODULE_LOGON), wSubCmdID, cbBuffer, wPacketSize + Packet.GetDataSize());

				//提示信息
				ShowStatusInfo(TEXT("正在验证用户登录信息..."));
			}
			else if (Command.wSubCmdID == SUB_GG_S_LINK_FAILURE)
			{

			}
			else if (Command.wSubCmdID == SUB_GG_S_LINK_SHUTDOWN)
			{
...
//执行登录
bool CMissionLogon::PerformLogonMission(BYTE cbMissonType)
{
	//效验状态
	ASSERT(GetActiveStatus()==false);
	...
	//设置地址
	CMissionManager * pMissionManager=GetMissionManager();
	if (pMissionManager!=NULL) pMissionManager->SetCurrentServer(szCustomServer);

	CServiceManager *pServiceManager=CServiceManager::GetServiceManager();
	if(pServiceManager!=NULL) pServiceManager->SetCurrentServer(szCustomServer);
...
//显示状态
VOID CMissionLogon::ShowStatusInfo(LPCTSTR pszStatusInfo)
{
	//创建窗口
	...
	m_pDlgStatus->ShowStatusWnd(pszStatusInfo,pPlatformFrame->m_hWnd,IDM_GIVE_UP_CONNECT);

	return;
}

//登录处理
bool CMissionLogon::OnSocketMainLogon(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	switch (wSubCmdID)
	{
...
//系统处理
bool CMissionLogon::OnSocketMainSystem(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	switch( wSubCmdID )
	{
...
//玩家服务
bool CMissionLogon::OnSocketMainService(WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	return true;
}

//房间列表
bool CMissionLogon::OnSocketMainServerList(WORD wSubCmdID, VOID* pData, WORD wDataSize)
{
	switch (wSubCmdID)
	{
...
bool CMissionLogon::OnSocketSubLogonSuccess(VOID * pData, WORD wDataSize)
{
	...
	//pGlobalUserData->dwStationID=pLogonSuccess->dwStationID;
...
//动作消息
bool CMissionLogon::OnSocketSubActionMessage(VOID * pData, WORD wDataSize)
{
	//变量定义
...
//关闭状态
VOID CMissionLogon::CloseStatusInfo()
{
	//关闭提示
	if ((m_pDlgStatus!=NULL)&&(m_pDlgStatus->m_hWnd!=NULL))
	{
...
//恢复界面
VOID CMissionLogon::RestoreShow()
{
	//显示界面
	ShowLogon();
...
//安全提醒
bool CMissionLogon::OnSocketSubSafePrompt(VOID * pData, WORD wDataSize)
{
	//RestoreShow();
...
//激活帐号
bool CMissionLogon::OnSocketSubActivateAccounts(VOID * pData, WORD wDataSize)
{
	//RestoreShow();
...
//人数显示
bool CMissionLogon::OnSocketSubShowOnLine(VOID * pData, WORD wDataSize)
{
	//效验参数
...
//后台升级
bool CMissionLogon::OnSocketSubUpdateSilent(VOID * pData, WORD wDataSize)
{
	return true;
}
//////////////////////////////////////////////////////////////////////////
