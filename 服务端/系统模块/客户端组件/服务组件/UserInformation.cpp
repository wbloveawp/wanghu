...
//接口查询
VOID * __cdecl CUserInformation::QueryInterface(REFGUID Guid, DWORD dwQueryVer)
{
...
//加载信息
bool __cdecl CUserInformation::LoadInformation()
{
	//变量定义
	...
	try
	{
		...
		//打开文件
		StgOpenStorage(INFORMATION_FILE,NULL,STGM_READ|STGM_SHARE_EXCLUSIVE,0,0,&pIStorageRoot);
...
//设置接口
bool __cdecl CUserInformation::SetUserCompanionSink(IUnknownEx * pIUnknownEx)
{
	//设置接口
	if (pIUnknownEx!=NULL)
	{
...
//枚举关系
tagCompanionInfo * __cdecl CUserInformation::EmunCompanionInfo(WORD wIndex)
{
	//枚举子项
...
//寻找关系
tagCompanionInfo * __cdecl CUserInformation::SearchCompanionInfo(DWORD dwUserID)
{
	//搜索关系
	for (INT_PTR i=0;i<m_CompanionInfoActive.GetCount();i++)
	{
...
//设置关系
tagCompanionInfo * __cdecl CUserInformation::InsertCompanionInfo(IClientUserItem * pIClientUserItem, BYTE cbCompanion)
{
	//效验参数
	...
	if (pCompanionInfo==NULL)
	{
		...
		lstrcpynW(pCompanionInfo->szAccounts,pIClientUserItem->GetAccounts(),CountArray(pCompanionInfo->szAccounts));
		lstrcpynW(pCompanionInfo->szUserNote,pIClientUserItem->GetUserNoteInfo(),CountArray(pCompanionInfo->szUserNote));
	...
	else
	{
		...
		if ((bModify==false)&&(wcscmp(pCompanionInfo->szAccounts,pIClientUserItem->GetAccounts())!=0)) bModify=true;
		if ((bModify==false)&&(wcscmp(pCompanionInfo->szUserNote,pIClientUserItem->GetUserNoteInfo())!=0)) bModify=true;
	...
	try
	{
		...
		// replace func
		_snwprintf_s(szStorageName,CountArray(szStorageName),L"%ld",dwUserID);

		//打开文件
		StgOpenStorage(INFORMATION_FILE,NULL,STGM_READWRITE|STGM_SHARE_EXCLUSIVE,0,0,&pIStorageRoot);
		if (pIStorageRoot==NULL) StgCreateDocfile(INFORMATION_FILE,STGM_CREATE|STGM_READWRITE|STGM_SHARE_EXCLUSIVE,0,&pIStorageRoot);
